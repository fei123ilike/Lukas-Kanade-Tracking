...
#test = correlate(img,[[0,1,0],[1,0,1],[0,1,0]])
#test2 = convolve(img,[[0,1,0],[1,0,1],[0,1,0]])
#print(test)
#print(test2)
...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        ...
        g = np.linalg.inv(S + Lambda * Identity) @ X @ Y

        fig3d = plt.figure()
        ax3d = fig3d.add_subplot(111, projection='3d')
        ax3d.plot_surface(dpx.reshape(dsize), dpy.reshape(dsize),
                          g.reshape(dsize), cmap=plt.get_cmap('coolwarm'))
        plt.title('lambda=0,without correlation')
...
#        Identity = np.identity(N)
        ...
        ax3d.plot_surface(dpx.reshape(dsize), dpy.reshape(dsize),
                          g.reshape(dsize), cmap=plt.get_cmap('coolwarm'))
        plt.title('lambda=1,without correlation')
        
        #with correlate
        Lambda1 = 0
#        S = X @ X.T
#        Identity = np.identity(N)
        g = np.linalg.inv(S + Lambda1 * Identity) @ X @ Y
        g_kernel = g.reshape(dsize)
        lena = correlate(img,g_kernel)
        lena_eye = lena[gnd_p[1]:gnd_p[1]+dsize[0],gnd_p[0]:gnd_p[0]+dsize[1]]
        

        fig3d = plt.figure()
        ax3d = fig3d.add_subplot(111, projection='3d')
        ax3d.plot_surface(dpx.reshape(dsize), dpy.reshape(dsize),
                          lena_eye, cmap=plt.get_cmap('coolwarm'))
        plt.title('lambda=0, with correlation')
        
        figure = plt.figure()
        plt.imshow(lena,cmap='gray')
        plt.title('lambda=0, with correlation, the whole image')
...
#        S = X @ X.T
#        Identity = np.identity(N)
        g = np.linalg.inv(S + Lambda2 * Identity) @ X @ Y
        g_kernel = g.reshape(dsize)
        lena = correlate(img,g_kernel)
        lena_eye = lena[gnd_p[1]:gnd_p[1]+dsize[0],gnd_p[0]:gnd_p[0]+dsize[1]]
        ...
        ax3d.plot_surface(dpx.reshape(dsize), dpy.reshape(dsize),
                          lena_eye, cmap=plt.get_cmap('coolwarm'))
        plt.title('lambda=1, with correlation')
        
        figure = plt.figure()
        plt.imshow(lena,cmap='gray')
        plt.title('lambda=1, with correlation, the whole image')
...
#        S = X @ X.T
#        Identity = np.identity(N)
        g = np.linalg.inv(S + Lambda3 * Identity) @ X @ Y
        g_kernel = g.reshape(dsize)
        lena = convolve(img,g_kernel)
        lena_eye = lena[gnd_p[1]:gnd_p[1]+dsize[0],gnd_p[0]:gnd_p[0]+dsize[1]]
        ...
        ax3d.plot_surface(dpx.reshape(dsize), dpy.reshape(dsize),
                          lena_eye, cmap=plt.get_cmap('coolwarm'))
        plt.title('lambda=0, with regular convolve')
        
        figure = plt.figure()
        plt.imshow(lena,cmap='gray')
        plt.title('lambda=0, with regular convolve, the whole image')
        
        print('you got here 3')
        
        Lambda4 = 1
#        S = X @ X.T
#        Identity = np.identity(N)
        g = np.linalg.inv(S + Lambda4 * Identity) @ X @ Y
        g_kernel = g.reshape(dsize)
        lena = convolve(img,g_kernel)
        lena_eye = lena[gnd_p[1]:gnd_p[1]+dsize[0],gnd_p[0]:gnd_p[0]+dsize[1]]
        ...
        ax3d.plot_surface(dpx.reshape(dsize), dpy.reshape(dsize),
                          lena_eye, cmap=plt.get_cmap('coolwarm'))
        plt.title('lambda=1, with regular convolve')
        
        figure = plt.figure()
        plt.imshow(lena,cmap='gray')
        plt.title('lambda=1, with regular convolve, the whole image')
        
        print('you got here 4')
        
        #covolve with flip
        Lambda5 = 0
#        S = X @ X.T
#        Identity = np.identity(N)
        g = np.linalg.inv(S + Lambda5 * Identity) @ X @ Y
        g_kernel = g.reshape(dsize)
        g_kernel = np.rot90(g_kernel,2)
        lena = convolve(img,g_kernel)
        lena_eye = lena[gnd_p[1]:gnd_p[1]+dsize[0],gnd_p[0]:gnd_p[0]+dsize[1]]
        ...
        ax3d.plot_surface(dpx.reshape(dsize), dpy.reshape(dsize),
                          lena_eye, cmap=plt.get_cmap('coolwarm'))
        plt.title('lambda=0, with special convolve(flip the index)')
        
        figure = plt.figure()
        plt.imshow(lena,cmap='gray')
        plt.title('lambda=0, with special convolve, the whole image')
        print('you got here 5')
        
        Lambda6 = 1
#        S = X @ X.T
#        Identity = np.identity(N)
        g = np.linalg.inv(S + Lambda6 * Identity) @ X @ Y
        g_kernel = g.reshape(dsize)
        g_kernel = np.rot90(g_kernel,2)
        lena = convolve(img,g_kernel)
        lena_eye = lena[gnd_p[1]:gnd_p[1]+dsize[0],gnd_p[0]:gnd_p[0]+dsize[1]]
        ...
        ax3d.plot_surface(dpx.reshape(dsize), dpy.reshape(dsize),
                          lena_eye, cmap=plt.get_cmap('coolwarm'))
        plt.title('lambda=1, with special convolve(flip the index)')
        
        figure = plt.figure()
        plt.imshow(lena,cmap='gray')
        plt.title('lambda=1, with special convolve, the whole image')
        print('you got here 6')
